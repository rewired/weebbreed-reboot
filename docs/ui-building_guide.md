...
- **Tick length control parity.** This guide states that tick length remains façade-managed and is not user-adjustable from the UI.【F:docs/ui-building_guide.md†L392-L392】 However, the component documentation still references a tick-length slider that dispatches `time.setTickLength`, implying direct user control.【F:docs/ui/ui-components-desciption.md†L31-L35】 We need to reconcile whether the UI should expose that slider or update the component spec to match this guide.
- **Frontend state orchestration.** The state management section here asserts a socket-backed Zustand store as the single source of truth, with intent dispatchers merely bridging to the façade.【F:docs/ui-building_guide.md†L366-L371】 In contrast, the component documentation describes `App.tsx` mutating local copies of `gameData` via `handle*` functions, effectively reintroducing client-side simulation logic.【F:docs/ui/ui-components-desciption.md†L76-L82】 That older description should be updated (or this guide amended) so we have one authoritative architecture.
- **Workforce & finance interaction coverage.** These interaction notes document structure assignment, overtime policy updates, salary negotiations, and finance actions (selling inventory, adjusting utility/maintenance pricing) as active UI flows.【F:docs/ui-building_guide.md†L416-L417】 The component tables still flag those intents as “not yet wired” and call the finance view read-only.【F:docs/ui/ui-components-desciption.md†L63-L69】 We must align the other docs (and implementation) with the façade contract captured here or revise this guide accordingly.
